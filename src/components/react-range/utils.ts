...
export function normalizeValue(
  ...
  // and the given value
  let remainder = Math.floor(value * BIG_NUM - min * BIG_NUM) %
    Math.floor(step * BIG_NUM);
  let closestLowerNum = Math.floor(value * BIG_NUM - Math.abs(remainder));
  let rounded = remainder === 0 ? value : closestLowerNum / BIG_NUM;
  // Values with a remainder `< step/2` are rounded to the closest lower value
  // while values with a remainder `= > step/2` are rounded to the closest bigger value
  let res = Math.abs(remainder / BIG_NUM) < step / 2
    ? rounded
    : rounded + step;
